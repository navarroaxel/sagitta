...
npm test        # full Jest suite (83 checks, 2 projects — must stay green)
...
| `src/lib/presets.ts`                                 | 7 preset models (incl. `r1` frame+truss, `r2` truss)                                                                                             |
...
Total ~83 checks. Component tests render with `@testing-library/react`; remember to add a test there when adding a canvas layer or fixing a rendering bug.
