...
describe('Test 1 – simply supported beam, UDL', () => {
  ...
  test('R0.rx = 0', () => expect(Math.abs(res.reactions[0].rx)).toBeLessThan(1e-6));

  test('mid-span M ≈ 125 (wL²/8)', () => {
    const mid = st.reduce((best, s) => Math.abs(s.x - 5) < Math.abs(best.x - 5) ? s : best);
    expect(near(mid.M, 125, 0.01)).toBe(true);
  });
  test('end M ≈ 0 at x=0', () => expect(Math.abs(st[0].M)).toBeLessThan(0.5));
  test('end M ≈ 0 at x=L', () => expect(Math.abs(st[st.length - 1].M)).toBeLessThan(0.5));
  test('Q at left end ≈ +50', () => expect(near(st[0].Q, 50, 0.01)).toBe(true));
  test('Q at right end ≈ -50', () => expect(near(st[st.length - 1].Q, -50, 0.01)).toBe(true));
});
...
describe('Test 2 – cantilever, point load at tip', () => {
  ...
  test('|R0.rm| = 100', () => expect(near(Math.abs(res.reactions[0].rm), 100)).toBe(true));
  test('|M| at support ≈ 100', () => expect(near(Math.abs(st[0].M), 100)).toBe(true));
  test('M at tip ≈ 0', () => expect(Math.abs(st[st.length - 1].M)).toBeLessThan(0.5));
...
describe('Test 3 – cantilever, UDL', () => {
  ...
  test('stable', () => expect(res.stable).toBe(true));
  test('R0.ry = 24 (=wL)', () => expect(near(res.reactions[0].ry, 24)).toBe(true));
  test('|R0.rm| = 48 (=wL²/2)', () => expect(near(Math.abs(res.reactions[0].rm), 48)).toBe(true));
  test('|M| at support ≈ 48', () => expect(near(Math.abs(st[0].M), 48)).toBe(true));
  test('M at tip ≈ 0', () => expect(Math.abs(st[st.length - 1].M)).toBeLessThan(0.1));
});
...
describe('Test 4 – moment release (hinge)', () => {
  ...
  test('stable', () => expect(res.stable).toBe(true));
  test('M at released end ≈ 0', () => expect(Math.abs(st1[0].M)).toBeLessThan(0.5));
});
...
describe('Test 5 – compound two-bay portal frame', () => {
  ...
  const nodeIndex: Record<string, number> = { A: 0, TL: 1, B: 2, H: 3, TM: 4, C: 5, TR: 6 };
  const model: SolverModel = {
    nodes: [
  ...
  const sumM_applied =
    (3 * (-8)) + (5 * (-15)) + (7 * (-10)) +  // point loads on TL-TM
  ...
  test('R_A.ry ≈ 11.90', () => expect(near(res.reactions[0].ry, 11.90, 0.05)).toBe(true));
  test('R_B.rx ≈ -14.00', () => expect(near(res.reactions[2].rx, -14.00, 0.05)).toBe(true));
  test('R_B.ry ≈ 54.60', () => expect(near(res.reactions[2].ry, 54.60, 0.05)).toBe(true));
  test('R_C.ry ≈ 26.50', () => expect(near(res.reactions[5].ry, 26.50, 0.05)).toBe(true));
});
