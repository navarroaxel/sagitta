...
function withZeros(stations: Station[], key: DiagramKey): Station[] {
  ...
  for (let i = 0; i < stations.length; i++) {
    ...
    if (i < stations.length - 1) {
      const v0 = stations[i][key], v1 = stations[i + 1][key];
      if (v0 * v1 < 0) {
        const t = v0 / (v0 - v1);
...
export function buildMemberDiagram(opts: BuildDiagramOptions): MemberDiagram {
  const { transform, nodeI, nodeJ, stations, key, scale, sideSign, globalMax, diagLen } = opts;
  const { toSX, toSY, k } = transform;

  // Member axis and normal in world coords
  const wiX = nodeI.x, wiY = nodeI.y;
  const wjX = nodeJ.x, wjY = nodeJ.y;
  ...
  const pixelPerUnit = globalMax > 1e-10
    ? (0.17 * diagLen * k * scale) / globalMax
    : 0;
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (i === points.length - 2 || (i + 1 < points.length - 1 && (points[i + 1].value >= 0) !== pos)) {
      currentGroup.push({
  ...
  const extIdx = points.reduce((bi, p, i) =>
    Math.abs(p.value) > Math.abs(points[bi].value) ? i : bi, 0);
...
export function quadGroupToSvgPoints(group: DiagramPolygon[]): string {
  ...
  const bwd = [...group].reverse().map((q) => `${q.base[0]},${q.base[1]}`).join(' ');
  return `${fwd} ${bwd}`;
}
