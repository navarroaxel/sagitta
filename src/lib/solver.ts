...
export function solveFrame(model: SolverModel): SolverResult {
  ...
  const matVec = (A: number[][], v: number[]): number[] => A.map((row) => row.reduce((s, a, i) => s + a * v[i], 0));
  ...
  loads.forEach((load) => {
    if (load.type === 'nodal') {
      ...
      const e = load.member, g = geo[e], T = transT(g);
  ...
  nodes.forEach((nd, i) => {
    const sp = nd.support;
    if (sp === 'pinned') { fixed[3 * i] = true; fixed[3 * i + 1] = true; }
    else if (sp === 'fixed') { fixed[3 * i] = fixed[3 * i + 1] = fixed[3 * i + 2] = true; }
    else if (sp === 'roller-v') { fixed[3 * i + 1] = true; }
    else if (sp === 'roller-h') { fixed[3 * i] = true; }
  });
  ...
  for (let col = 0; col < nf; col++) {
    ...
    if (Math.abs(aug[piv][col]) < 1e-9) { stable = false; break; }
